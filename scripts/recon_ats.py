...
def main(argv=None) -> int:
    ...
    p.add_argument("--domain", dest="domain_opt", default=None)
    p.add_argument("--html", help="Optional local HTML file to scan")
    args = p.parse_args(argv)
    domain = args.domain_opt or args.domain
    if not domain:
        p.error("domain required")
    print("candidates:")
    for url in careers_url_candidates(domain):
        print(" ", url)
    print("vendors:", ", ".join(ATS_PATTERNS))
    if args.html:
        html = Path(args.html).read_text(encoding="utf-8")
        matches = detect_ats(html, f"https://{domain}/")
        print("matches:")
        for m in matches:
