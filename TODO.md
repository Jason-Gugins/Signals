...
### 10. `needs` promoted one signal per document copy -- FIXED by 2abe4fe
Measured with the demo market profile (`sample_soc_platform`): **15 identical `need_statement`
signals** from ONE sentence. Two causes, both verified: (a) Darktrace repeats the same "What we're
building" boilerplate block on all 5 stored articles; (b) each article is stored **3 times** because
content-addressed storage mints a new `documents` row whenever the page's dynamic bytes differ (the
3 copies have identical byte length and different sha256). 1 sentence x 5 articles x 3 copies = 15.
Fix: one candidate per distinct NORMALISED quote text, the newest document wins (ties broken by
largest `doc_id`, pinned by a test), provenance retained in evidence, and the natural key is now
`need:{domain}:{hash(quote)}` with `doc_id` removed so re-runs are idempotent. Live re-check with the
same profile: **15 -> 1** `need_statement`, `required_stack_demand` unchanged at 3 (Senior People
Advisor/Workday, Salesforce Developer (CPQ), Back Office Finance Platforms Lead).
Also fixed the dossier display, which printed `matched: we're building` (the EXTRACTOR's phrase) as
though it were the profile match; it now shows `profile reason: phrase:security investigation,
phrase:risky behavior, phrase:enterprise ai systems`, with the extractor phrase demoted to a
conditional `extractor_phrase:` line.
**Still OPEN (separate decision):** `rawstore` keeps 3 documents per URL per cycle because dynamic
page bytes change the content hash. The dedupe stops that inflating `needs`, but document-level
consumers and storage still see the copies.
- **Baseline** -- `pytest --collect-only` -> 2065 collected; the offline lane passes with only the known unmarked antibot TLS probe flaking (2048-2049 passed per run).
...
### 11. `rawstore` mints a new document per fetch when the response changes -- DECISION OPEN
Jason's framing (2026-09-16): *"rawstore still mints a new document per URL per cycle when the page's
dynamic bytes change. The dedupe keeps that out of your signals, but document-level consumers and
storage still see 3 copies of every article, so it's worth a decision of its own: keep only the newest
document per URL, or leave it and accept the storage growth."*

Measured repo-wide (2026-09-16): **2,391 documents, 159 URLs with more than one row, 395 excess rows**
-- roughly 16% of all stored documents are duplicates. Worst offenders: `https://clutch.co/` x67,
Google News topic feeds x26/x24, the Workday CXS `/jobs` endpoint x17, `https://gartner.com/` x14.

**The naive fix is not safe**, and the stored byte sizes prove it: for one URL the response genuinely
differs between fetches -- `clutch.co` 573..6,123 bytes; `gartner.com` 672..313,013 including repeated
9,999-byte block/challenge pages; Google News topic feeds ~203K..229K as the feed evolves; and the
Workday `/jobs` URL 3,838..12,710 because that POST endpoint carries the PAGE OFFSET IN THE REQUEST
BODY, so every list page shares one URL. "Keep the newest row per URL" would therefore delete distinct
Workday list pages and legitimately different feed snapshots.

Options: (a) leave as is and accept the growth -- defensible short-term, since finding 10 now stops it
inflating signals; (b) dedupe only when bodies are EQUIVALENT (near-identical size + content, i.e. the
same page re-fetched from a dynamic response) and never for request-body-varying endpoints; (c) store a
request-body digest alongside the URL so POST-derived pages are distinguishable, then dedupe on
(source, url, request digest). Recommendation: (c) then (b).
...
- **Workday coverage converged on live data (2026-09-16)** -- three `intel darktrace.com --force` cycles took job descriptions 0 -> 10 -> 20 -> 60 -> **77 of 77 OPEN postings (100%)**. The runner's budget line unwound with it: `dropped 57 ... over the per-cycle budget of 10` -> `dropped 17 ... over a budget of 40` -> **no line at all** (nothing queued beyond the cap). Cycle-4's 40 GETs were 40 distinct postings (zero wasted slots, no cross-page duplicates), every described row kept its `posted_at` (77/77, the C4 merge), `department` stayed 0 (no such field in Workday's payload per C1), the 7 undescribed rows are CLOSED postings that never appear on a list page, and the account table did not move (389 -> 389) because the fanout globals stayed opted out (11 sources ran instead of 14; the cycle made 49 requests where the pre-fix run made 247). The `needs` gap count grew 7 -> 12 -> 17 as feed/article documents accumulated, which is the reporting fix doing its job.
